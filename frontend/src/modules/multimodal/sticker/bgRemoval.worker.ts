...
// 模型文件同源托管（frontend/public/models/），避免依赖 Hugging Face Hub 可达性；
// 保留远端 cache key 口径：Transformers.js 先检查旧 HF 缓存键再取本地路径。
env.allowLocalModels = true;
env.allowRemoteModels = false;
env.localModelPath = LOCAL_MODEL_PATH;

// U²-Net 输出软 matte，其低置信度尾部常含原始背景色；不压缩会在贴纸描边上方
// 渲染出灰色光晕。压缩尾部同时保留头发/毛发周围的羽化过渡。
...
function postProgress(id: number, phase: StickerBackgroundRemovalProgress['phase'], progress?: number) {
