...
export const JournalTextToolbar: React.FC<JournalTextToolbarProps> = ({
  ...
  const adjustSize = (delta: number) => {
    const nextW = Math.round(Math.min(18, Math.max(3, item.w + delta)) * 10) / 10;
    onUpdate({ w: nextW });
  };
  ...
  return (
    <div
      ...
      className="flex flex-col gap-1.5 p-1.5 rounded-xl bg-paper/95 backdrop-blur-md shadow-xl border border-paper-grid/60 text-ink text-xs select-none pointer-events-auto"
    >
      {/* 第一行：主要快捷操作（编辑文案、字体切换、横竖排、字号微调、图层控制与删除） */}
      <div className="flex items-center gap-1">
        ...
        <Tooltip content="编辑文字内容 (或双击文字)">
          <button
            ...
            disabled={disabled}
            onClick={onOpenEdit}
            className="flex items-center gap-1 px-2 py-1 rounded-md text-ink-light hover:text-accent hover:bg-paper-grid/40 active:scale-[0.96] transition-[background-color,color,transform] duration-150 ease-out font-medium"
          >
        ...
        <div className="w-px h-3.5 bg-paper-grid/60 my-auto" />
        ...
        <div className="relative flex items-center">
          <select
            ...
            disabled={disabled}
            aria-label="选择字体"
            className="h-6 pl-2 pr-5 rounded-md border border-paper-grid/60 bg-paper/90 text-xs text-ink font-sans outline-none hover:border-accent/60 focus:border-accent cursor-pointer transition"
          >
        ...
        <Tooltip content={isVertical ? '切换为横向排版' : '切换为纵向排版'}>
          <button
            ...
            className={`flex items-center gap-0.5 px-1.5 py-1 rounded-md text-xs active:scale-[0.96] transition-[background-color,color,transform] duration-150 ease-out ${
              isVertical
                ? 'bg-accent/15 text-accent font-medium'
                : 'text-ink-light hover:text-accent hover:bg-paper-grid/40'
            }`}
          >
            <span className="font-mono text-[11px] leading-none px-0.5">
              {isVertical ? '竖' : '横'}
            </span>
          </button>
        </Tooltip>
        ...
        {/* 字号加减步进器 */}
        <div className="flex items-center gap-0.5 bg-paper-grid/30 rounded-md p-0.5">
          <Tooltip content="缩小字号">
            <button
              type="button"
              disabled={disabled || item.w <= 3}
              onClick={() => adjustSize(-1)}
              className="p-1 rounded text-ink-light hover:text-accent hover:bg-paper active:scale-[0.96] transition disabled:opacity-30"
            >
              <Minus size={11} strokeWidth={2} />
            </button>
          </Tooltip>
          <span className="text-[11px] font-mono text-ink-light px-1 select-none">
            {Math.round(item.w)}
          </span>
          <Tooltip content="放大字号">
            <button
              type="button"
              disabled={disabled || item.w >= 18}
              onClick={() => adjustSize(1)}
              className="p-1 rounded text-ink-light hover:text-accent hover:bg-paper active:scale-[0.96] transition disabled:opacity-30"
            >
      ...
      <div className="flex items-center gap-1.5 pt-0.5 px-0.5 border-t border-paper-grid/40">
        <span className="text-[10px] text-ink-faint mr-0.5 select-none">墨色:</span>
            ...
            return (
              <Tooltip key={preset.name} content={`${preset.name} (${preset.color})`}>
                <button
                  ...
                  className={`relative w-4 h-4 rounded-full transition-all duration-150 ease-out active:scale-[0.92] ${
