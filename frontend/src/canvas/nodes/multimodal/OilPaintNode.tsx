...
import type { OilPaintState, OilPaintStyle } from './engines/oilpaint';
import type { WetPaintSession } from './engines/oilpaint/engine';
...
const OilPaintNodeInner: React.FC<OilPaintNodeProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { WetPaintSession } = await import('./engines/oilpaint');
        if (cancelled) return;
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (session && sessionStatus === 'ready') {
        ...
        const { renderWetPaintFromImage } = await import('./engines/oilpaint');
