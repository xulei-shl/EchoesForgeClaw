...
} from './engines/watercolor';
import type { WatercolorSession } from './engines/watercolor/engine';
...
const WatercolorBrushNodeInner: React.FC<WatercolorBrushNodeProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { WatercolorSession, getWatercolorDimensions } = await import('./engines/watercolor');
        if (cancelled) return;
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const { renderWatercolorArt } = await import('./engines/watercolor');
