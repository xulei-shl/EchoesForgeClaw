import type { NodePortType, NodeRunSettings } from '../../shared/types';
import {
  TEXT_ROLE,
  bookMetadataText,
...
import type { PortTypesLookup } from '../nodes/_shared/nodeTypes';
...
export type { PortTypesLookup } from '../nodes/_shared/nodeTypes';
 ...
 * - 图书元数据：文本与封面开关独立（「包含图书元数据」控文本、「加载图书封面图片」控封面，
 *   任一开启即解析图书）；直接连线的 book_info 优先，未直接连线时沿连线向上追溯实际连通的
 *   book_info，无连通时才回退画布根节点；
