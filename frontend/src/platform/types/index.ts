...
export type CanvasNodeType =
  ...
  | 'web_search'
  /** 图书小票节点：热敏纸风格小票、借书卡、古籍排版等书目推荐卡片（多模态工具） */
  | 'receipt_printer'
  /** 图书卡片节点：书目元数据与封面填入 HTML 推广卡片模板，浏览器端截图为图片输出（多模态工具） */
  | 'book_card'
  /** 邮票制作节点：锯齿邮票框自由截取，生成带打孔边缘与边框的邮票图片（多模态工具） */
  | 'stamp_cutter'
  /** 抠图节点：AI 抠图移除背景，输出透明通道 PNG 或纯色背景图（多模态工具） */
  | 'image_bg_remove'
...
export interface AgentFile {
  ...
  path: string;
  /**
   * 服务端文件是否仍存在（仅 /chat/files AI 产物列表携带；
   * manifest 历史条目对应文件被删除时为 false，面板侧过滤不展示）
   */
  exists?: boolean;
  /**
   * 文件修改时间（unix 毫秒；仅 /chat/files AI 产物列表携带，
