...
const NAV_ITEMS = [
  ...
  { to: '/admin/prompt-templates', label: '本地提示词', icon: FileText },
  { to: '/admin/prompts', label: '提示词库', icon: BookOpen },
  { to: '/admin/skills', label: 'Skills 技能库', icon: Workflow },
...
const TITLE_MAP: Record<string, string> = {
  ...
  '/admin/prompt-templates': '本地提示词',
  '/admin/prompts': '提示词库',
  '/admin/skills': 'Skills 技能库',
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto" aria-label="管理后台导航">
          {NAV_ITEMS.map((item) => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-sans transition-colors active:scale-[0.97] border border-transparent ${
                  isActive
