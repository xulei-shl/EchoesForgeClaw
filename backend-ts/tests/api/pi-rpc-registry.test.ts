...
describe('piProcessRegistry（RPC 子进程注册表）', () => {
  ...
  it('注册后可按 userId+workspaceId 复合 key 写回 extension_ui_response', () => {
    ...
    unregs.push(registerPiProcess(7, 'ws-1', makeEntry(stdin, 101, noopKill)));
  ...
  it('复合 key 隔离：同名 workspace 不同 userId 不串扰', () => {
    ...
    unregs.push(registerPiProcess(1, 'ws-shared', makeEntry(a.stdin, 1, noopKill)));
    unregs.push(registerPiProcess(2, 'ws-shared', makeEntry(b.stdin, 2, noopKill)));
  ...
  it('confirmed/cancelled 应答与组合一致性', () => {
    ...
    unregs.push(registerPiProcess(3, 'ws-c', makeEntry(yes.stdin, 3, noopKill)));
    unregs.push(registerPiProcess(3, 'ws-d', makeEntry(no.stdin, 3, noopKill)));
  ...
  it('注销后（ended）写回返回 false（404 语义）', () => {
    ...
    const unreg = registerPiProcess(9, 'ws-dead', makeEntry(stdin, 9, noopKill));
    unreg();
  ...
  it('同一 key 重新注册后覆盖旧进程（新一轮 spawn 顶替）', () => {
    ...
    let oldKilled = 0;
    unregs.push(registerPiProcess(5, 'ws-cycle', makeEntry(old.stdin, 501, () => { oldKilled += 1; })));
    const unregFresh = registerPiProcess(5, 'ws-cycle', makeEntry(fresh.stdin, 502, noopKill));
  ...
  it('过期轮的注销不误删新进程的注册项（identity 校验）', () => {
    ...
    const fresh = makeFakeStdin();
    const unregStale = registerPiProcess(8, 'ws-ident', makeEntry(stale.stdin, 801, noopKill));
    const unregFresh = registerPiProcess(8, 'ws-ident', makeEntry(fresh.stdin, 802, noopKill));
  ...
  it('killPiProcess 终止并注销：写回随即 404，kill 回调被调用', async () => {
    const { stdin } = makeFakeStdin();
    let killed = 0;
    const unreg = registerPiProcess(6, 'ws-kill', makeEntry(stdin, 6, () => {
      killed += 1;
    }));
    unregs.push(unreg);
    expect(await killPiProcess(6, 'ws-kill')).toBe(true);
    expect(killed).toBe(1);
    expect(sendExtensionUiResponse(6, 'ws-kill', { id: 'u1', value: 'x' })).toBe(false);
    // 二次 kill 无效果（已注销）
    expect(await killPiProcess(6, 'ws-kill')).toBe(false);
    expect(killed).toBe(1);
  });
});
