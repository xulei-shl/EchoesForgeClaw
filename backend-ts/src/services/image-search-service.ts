/**
 * 图片检索服务（多模态工具：Unsplash / Pixabay 图片检索节点）。
 *
 ...
 *
 * 两个 API 的凭据均由系统设置提供（不在 .env 配置），经路由读取后传入。
 * 返回统一归一化的 ImageSearchItem，前端不感知两家 API 的字段差异。
 */

const UNSPLASH_API = 'https://api.unsplash.com';
const PIXABAY_API = 'https://pixabay.com/api';

/** 外部检索请求超时（ms）：两家均为轻量 JSON 接口，20s 足够。 */
const SEARCH_TIMEOUT_MS = 20_000;

/** 单页最大条数（Unsplash random 单次上限 30；Pixabay per_page 上限 200）。 */
...
export type ImageSearchProvider = 'unsplash' | 'pixabay';
