...
class LibraryCardHTMLGenerator:
    ...
    def __init__(self, config: Dict):
        ...
        self.handwriting_fonts_cn = config.get(
            'handwriting_fonts_cn',
            [f for f in self.handwriting_fonts if not f.endswith('-en')],
        )
        self.handwriting_fonts_en = config.get(
            'handwriting_fonts_en',
            [f for f in self.handwriting_fonts if f.endswith('-en')],
        )
    ...
    def generate_borrower_records_html(self, records: List[BorrowerRecord]) -> str:
        ...
        cn_records = [(i, r) for i, r in enumerate(records) if self._is_chinese_name(r.name)]
        en_records = [(i, r) for i, r in enumerate(records) if not self._is_chinese_name(r.name)]

        cn_font_sequence = self._generate_font_sequence(len(cn_records), self.handwriting_fonts_cn)
        en_font_sequence = self._generate_font_sequence(len(en_records), self.handwriting_fonts_en)

        font_map: Dict[int, str] = {}
        for (i, _), font_class in zip(cn_records, cn_font_sequence):
            font_map[i] = font_class
        for (i, _), font_class in zip(en_records, en_font_sequence):
            font_map[i] = font_class

        for idx, record in enumerate(records):
            display_date = record.format_date_for_display()
            font_class = font_map[idx]
    ...
    def _generate_font_sequence(self, count: int, fonts: List[str]) -> List[str]:
        """
        为单张卡片生成字体序列，尽量保证不重复
        """
        pool = list(fonts) if fonts else ['font-handwriting-cn']
        random.shuffle(pool)
        sequence: List[str] = []
