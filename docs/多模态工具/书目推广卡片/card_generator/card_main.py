    ...
    def _write_recommendation_results_task(self, df: pd.DataFrame, success_barcodes: List[str] = None) -> int:
        ...
        try:
            ...
            if success_barcodes:
                ...
                # 兜底路径: 按与卡片生成一致的筛选条件(人工评选=通过)过滤
                manual_column = '人工评选'
                if manual_column in df.columns:
                    df_to_write = df[df[manual_column].astype(str).str.strip() == '通过']
                else:
                    logger.warning(f"未找到'{manual_column}'列，无法确定入选记录，跳过数据库写入")
                    return 0

            # 写入数据(仅写入筛选通过且成功生成卡片的记录)
...
def main():
    ...
    if args.output_dir:
        if 'output' not in card_config:
            card_config['output'] = {}
        card_config['output']['base_dir'] = args.output_dir
