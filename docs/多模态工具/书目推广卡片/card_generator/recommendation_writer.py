...
class RecommendationResultsWriter:
    ...
    def write_recommendation_results(self, df: pd.DataFrame) -> int:
        ...
        Args:
            df: 筛选后的DataFrame(已由调用方按"人工评选=通过"及卡片生成成功筛选)
        ...
        try:
            ...
            for excel_col, db_field in self.recommendation_mapping.items():
                if excel_col in row:
                    ...
                    # 始终写入映射字段(即使为空值),保证数据库与Excel最新状态一致,
                    # 避免旧值残留导致已取消评选的书目无法被重新纳入候选
    ...
    def _insert_or_update_record(self, record_data: Dict) -> bool:
        ...
        try:
            ...
            # 构建UPDATE子句(排除主键barcode/evaluation_batch及创建时间created_at,
            # created_at仅在首次插入时写入,更新时保留原值)
            update_clauses = []
            for col in columns:
                if col not in ['barcode', 'evaluation_batch', 'created_at']:
