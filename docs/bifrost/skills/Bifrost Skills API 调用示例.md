...
本示例演示如何调用 Bifrost 的 Skills API，包含：

- Management API（需 **Basic Auth**，用管理员账号密码）
- Serving / Marketplace API（**公开**，无需认证）

## 凭证与配置

本实例为自部署版本，认证方式如下（见 `../prompts-api/bitfrost后端管理密钥.md`）：

| 接口类型 | 认证方式 |
| -------- | -------- |
| 推理 API `/v1/*` | 无需认证 |
| 管理 API `/api/*`（含 Skills） | **Basic Auth**：`base64(admin:password)` |
| Serving API `/api/skills/serve/*` | 公开，无需认证 |
...
```bash
echo -n "admin:Yfzjlxy_0527" | base64
# 得到：YWRtaW46WWZ6amx4eV8wNTI3
# 请求头：Authorization: Basic YWRtaW46WWZ6amx4eV8wNTI3
```

---

## Python 示例（bifrost_skills_example.py）
...
if __name__ == "__main__":
    import urllib.parse  # noqa: E402  (供 list_skills 搜索参数使用)
    main()
```
...
```bash
# 安装依赖（示例使用标准库，无需第三方包）
python bifrost_skills_example.py
```
...
```python
import requests
r = requests.get(url, headers={"Authorization": f"Basic {BASIC_AUTH}"})
```
...
```bash
# Basic Auth 头
AUTH="Basic $(printf '%s' 'admin:Yfzjlxy_0527' | base64)"

# 1. 列出
curl -X GET "http://10.40.92.18:8080/api/skills?limit=50" -H "$AUTH"

# 2. 单个（含文件元数据）
curl -X GET "http://10.40.92.18:8080/api/skills/SKILL_ID" -H "$AUTH"

# 3. 版本历史
curl -X GET "http://10.40.92.18:8080/api/skills/SKILL_ID/versions" -H "$AUTH"

# 4.1 完整 ZIP（公开）
curl -o skill.zip "http://10.40.92.18:8080/api/skills/serve/SKILL_NAME/download.zip"

# 4.2 全部 ZIP（公开）
curl -o all.zip "http://10.40.92.18:8080/api/skills/serve/all/download.zip"

# 4.3 单文件（公开）
curl -o file.md "http://10.40.92.18:8080/api/skills/serve/SKILL_NAME/files/references/style-guide.md"
```
