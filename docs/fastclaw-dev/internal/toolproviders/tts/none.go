...
// The tool-registration layer (internal/agent/tools/tts.go) detects "none"
// anywhere in the chain and skips registering tts at all, so the model falls
// back to whatever native audio capability it has (or does without).
//
...
// Execute should never be reached: tts registration short-circuits on "none"
// before the chain runs. The error is defensive — if someone wires the chain
// a new way and bypasses the skip, the failure should surface loudly rather
