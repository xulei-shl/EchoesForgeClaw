...
>
> **在既有模块内新增「节点类型」**（如文本聚合、未来的文本判断）——包括端口声明、占位符机制、
> 可复用公共组件（`CanvasNode` / `NodeSettingsPopover` / `PlaceholderPanel` 等）与接入步骤、注意事项，
> 见 [《节点输入接线（连线即输入 · 端口类型匹配 · 占位符模板）》](./节点输入输出声明式接线.md) 第 3 节。
